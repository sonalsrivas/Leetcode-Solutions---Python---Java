        ...
        def BFSbuildingToHousingCells(x, y, relevantHousingCellsMapDistance):
            ...
            while q:
                ...
                for v, w in neighbours:
                    v += i;
                    w += j
                    if -1 < v < m and -1 < w < n and grid[v][w] == 0 and (v, w) in relevantHousingCellsMapDistance and (
                    v, w) not in relevantHousingCellsMapDistance_temp:
                        q.append((v, w, level + 1))
                        distance = level + 1
                        relevantHousingCellsMapDistance_temp[(v, w)] = relevantHousingCellsMapDistance[
                                                                           (v, w)] + distance
