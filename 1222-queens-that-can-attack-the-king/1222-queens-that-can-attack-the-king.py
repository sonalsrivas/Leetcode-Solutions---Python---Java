...
class Solution:
    def queensAttacktheKing(self, queens: List[List[int]], king: List[int]) -> List[List[int]]:
        ...
        queenInRow=defaultdict(set)
        queenInCol=defaultdict(set)
        queenInDiagLeftRight=defaultdict(set)
        queenInDiagRightLeft=defaultdict(set)
        ...
        # To_Do :::: for diagonals
        for i,j in queens:
            queenInDiagLeftRight[i-j].add((i,j))
            queenInDiagRightLeft[i+j].add((i,j))
        ...
        while -1<new_i<n and  -1<new_j<n:
            if (new_i, new_j) in queenInDiagLeftRight[diagonal_difference]:
        ...
        new_i, new_j=i-1, j+1
        while -1<new_i<n and  -1<new_j<n:
            if (new_i, new_j) in queenInDiagRightLeft[diagonal_sum]:
                queensThatCanAttackList.append([new_i, new_j])
                break
            new_i-=1
            new_j+=1
